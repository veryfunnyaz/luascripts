...
do --ANCHOR Visuals
	...
	for i = 2, #constants + 1 do
		coroutine.wrap(function()
			...
			if icon == "C4" or replicatedStorage.Weapons:FindFirstChild(icon) then -- we only need this for now
				if thing and thing:find("rbxassetid") then
					thing = thing:gsub("rbxassetid://", "")
					local req_data = game:HttpGet("https://assetdelivery.roblox.com/v1/assetId/" .. thing)
					local endpoint = httpService:JSONDecode(req_data).location
